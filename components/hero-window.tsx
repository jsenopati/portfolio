import { Win7Window } from './win7-window'

type HeroWindowProps = {
  defaultX?: number
  defaultY?: number
}

export const HeroWindow = ({ defaultX, defaultY }: HeroWindowProps) => {
  return (
    <Win7Window
      title="Julien Senopati — Portfolio"
      defaultX={defaultX}
      defaultY={defaultY}
    >
      <div className="window-body">
        <div className="flex w-96 flex-col items-center gap-4 bg-gray-900 py-4">
          <h1 className="flex items-center gap-2 bg-linear-to-r from-emerald-500 to-blue-300 bg-clip-text text-4xl font-bold text-transparent">
            Hello, I&apos;m Julien.
            <span className="animate-bounce text-black">👋</span>
          </h1>
          <div className="h-1 w-full bg-linear-to-r from-blue-300 to-emerald-500" />
          <div className="flex flex-row justify-center gap-12 py-2">
            {/* GitHub */}
            <a
              href="https://github.com/jsenopati"
              target="_blank"
              rel="noreferrer"
            >
              <svg
                className="h-12 w-12 rounded-3xl border-2 border-transparent transition duration-300 hover:scale-125 hover:animate-pulse hover:border-teal-500"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
              >
                <defs>
                  <linearGradient
                    id="gh-grad"
                    x1="0%"
                    y1="0%"
                    x2="100%"
                    y2="0%"
                  >
                    <stop offset="0%" style={{ stopColor: '#9333EA' }} />
                    <stop offset="100%" style={{ stopColor: '#F9A8D4' }} />
                  </linearGradient>
                </defs>
                <path
                  fill="url(#gh-grad)"
                  d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"
                />
              </svg>
            </a>

            {/* Email */}
            <a href="mailto:jasenopati@gmail.com">
              <svg
                className="h-12 w-12 rounded-3xl border-2 border-transparent transition duration-300 hover:scale-125 hover:animate-pulse hover:border-teal-500"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
              >
                <defs>
                  <linearGradient
                    id="email-grad"
                    x1="0%"
                    y1="0%"
                    x2="100%"
                    y2="0%"
                  >
                    <stop offset="0%" style={{ stopColor: '#F9A8D4' }} />
                    <stop offset="100%" style={{ stopColor: '#D8B4FE' }} />
                  </linearGradient>
                </defs>
                <path
                  fill="url(#email-grad)"
                  d="M10 .4C4.698.4.4 4.698.4 10s4.298 9.6 9.6 9.6 9.6-4.298 9.6-9.6S15.302.4 10 .4zM6.231 7h7.52c.399 0 .193.512-.024.643-.217.13-3.22 1.947-3.333 2.014s-.257.1-.403.1a.793.793 0 0 1-.402-.1L6.255 7.643C6.038 7.512 5.833 7 6.231 7zM14 12.5c0 .21-.252.5-.444.5H6.444C6.252 13 6 12.71 6 12.5V8.853c0-.092-.002-.211.172-.11l3.417 2.015a.69.69 0 0 0 .402.1c.146 0 .252-.011.403-.1l3.434-2.014c.174-.102.172.018.172.11V12.5z"
                />
              </svg>
            </a>

            {/* LinkedIn */}
            <a
              href="https://www.linkedin.com/in/julien-senopati/"
              target="_blank"
              rel="noreferrer"
            >
              <svg
                className="h-12 w-12 rounded-3xl border-2 border-transparent transition duration-300 hover:scale-125 hover:animate-pulse hover:border-teal-500"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
              >
                <defs>
                  <linearGradient
                    id="li-grad"
                    x1="0%"
                    y1="0%"
                    x2="100%"
                    y2="0%"
                  >
                    <stop offset="0%" style={{ stopColor: '#D8B4FE' }} />
                    <stop offset="100%" style={{ stopColor: '#DB2777' }} />
                  </linearGradient>
                </defs>
                <path
                  fill="url(#li-grad)"
                  d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"
                />
              </svg>
            </a>
          </div>
        </div>
      </div>
    </Win7Window>
  )
}
